...
class InfluencerUpdateRequest extends FormRequest
{
    ...
    {
        $inflId = $this->route('id'); // get current user ID
        return [
            'name'            => 'required|string|max:100|min:3|regex:/^[a-zA-Z\s]+$/',
            'email'           => ['required','email','regex:/^[a-zA-Z0-9.@]+$/', Rule::unique('influencers', 'email')->ignore($inflId),],
            'phone'           => 'required|string|regex:/^[6-9][0-9]{9}$/',
            'profile_image'   => 'sometimes|nullable|image|mimes:jpg,jpeg,png|max:500',
